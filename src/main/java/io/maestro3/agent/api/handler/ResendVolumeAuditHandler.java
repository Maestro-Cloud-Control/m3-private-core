...
import io.maestro3.agent.service.IVolumesChecker;
...
@Component
public class ResendVolumeAuditHandler extends AbstractM3ApiHandler<ResendVolumeAuditRequest, Void> {

    private Map<PrivateCloudType, IVolumesChecker> checkerMap;
    ...
    @Autowired
    public ResendVolumeAuditHandler(Set<IVolumesChecker> checkers,
                                    Set<ITenantRepository<? extends ITenant>> tenantRepos,
                                    Set<IRegionRepository<? extends IRegion<? extends ShapeConfig>>> regionRepos) {
        super(ResendVolumeAuditRequest.class, ActionType.RESEND_VOLUME_AUDIT);
        this.checkerMap = checkers.stream()
            .collect(Collectors.toMap(IVolumesChecker::getCloud, Function.identity()));
    ...
    @Override
    protected Void handlePayload(ActionType action, ResendVolumeAuditRequest request) {
        ...
        IVolumesChecker volumesChecker = checkerMap.get(privateCloudType);
        if (volumesChecker == null) {
            throw new IllegalArgumentException("Audit resend is not available for " + privateCloudType + " cloud");
        }
        volumesChecker.checkVolumes(tenant, region, request.getDataList());
        return null;
    }
